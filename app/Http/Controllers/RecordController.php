    ...
    public function postNew()
    {
        ...
        if ($this->validateRequiredFields($data)) {
            ...
            if ($record->isOK()) {
                $record->add($data);
                if ($record->commit()) {
                    return redirect('/new')->withSuccess('Client record inserted');
                } else {
                    return redirect('/new')->withError('Error inserting client record');
                }
            }
        } else {
            return redirect('/new')->withError('Data validation error.');
        }
    }
